...
void llama_model_loader::done_getting_tensors(bool partial) const {
    if (n_created > n_tensors) {
        throw std::runtime_error(format("%s: wrong number of tensors; created more than expected (%d > %d)", __func__, n_created, n_tensors));
    }
    if (n_created < n_tensors) {
        if (!partial) {
            throw std::runtime_error(format("%s: wrong number of tensors; expected %d, got %d", __func__, n_tensors, n_created));
        }
        LLAMA_LOG_INFO("%s: partial load: %d out of %d tensors loaded\n", __func__, n_created, n_tensors);
    }
    if (n_tensors_moved > 0) {
